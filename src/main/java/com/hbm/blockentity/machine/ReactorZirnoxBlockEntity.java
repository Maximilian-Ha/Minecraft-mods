...
public class ReactorZirnoxBlockEntity extends MachineBaseBlockEntity implements IControlReceiver, IFluidStandardTransceiverMK2 {
    ...
    /**
     * Die Brennstofftabelle. Im Original steht sie im Konstruktor der Blockentitaet und ist
     * damit erst gefuellt, sobald irgendwo ein ZIRNOX steht -- die JEI-Ansicht braucht sie
     * aber schon beim Start. Deshalb steht sie hier fuer sich und wird von beiden Seiten
     * angestossen.
     */
    public static void initFuelMap() {

        if(!fuelMap.isEmpty()) return;
    ...
    private void zirnoxDebris() {
        ...
        for(int i = 0; i < 2; i++) this.spawnDebris(DebrisType.EXCHANGER);

        for(int i = 0; i < 20; i++) {
            this.spawnDebris(DebrisType.CONCRETE);
            this.spawnDebris(DebrisType.BLANK);
        }

        for(int i = 0; i < 10; i++) {
            this.spawnDebris(DebrisType.ELEMENT);
            this.spawnDebris(DebrisType.GRAPHITE);
            this.spawnDebris(DebrisType.SHRAPNEL);
        }
    }

    private void spawnDebris(DebrisType type) {

        ZirnoxDebris debris = new ZirnoxDebris(this.level,
                this.worldPosition.getX() + 0.5D, this.worldPosition.getY() + 4D, this.worldPosition.getZ() + 0.5D, type);

        double motionX = this.level.random.nextGaussian() * 0.75D;
        double motionZ = this.level.random.nextGaussian() * 0.75D;
        double motionY = 0.01D + this.level.random.nextDouble() * 1.25D;

        /* Der Betonbrocken fliegt fast senkrecht, der Waermetauscher fast flach. */
        if(type == DebrisType.CONCRETE) {
            motionX *= 0.25D;
            motionY += this.level.random.nextDouble();
            motionZ *= 0.25D;
        }
