...
public class MaterialShapes {
    ...
    /**
     * Der Tag-Pfad aus Form und Material, also das Gegenstueck zum OreDictionary-Namen des
     * Originals: aus "ingot" + "Steel" wird hier "ingots/steel".
     */
    public String make(NTMMaterial mat) {
        return this.name() + "/" + mat.tagName;
    }

    /**
     * Der Tag selbst. Der Namensraum ist "c", der gemeinsame Raum aller Mods -- so findet der
     * Tiegel auch Barren anderer Mods, genau wie das Original sie ueber den OreDictionary fand.
     */
    public TagKey<Item> getTag(NTMMaterial mat) {
        return this.tagCache.computeIfAbsent(mat, m -> ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", this.make(m))));
    }

    private final Map<NTMMaterial, TagKey<Item>> tagCache = new HashMap<>();
}
