...
public class Fluids {
    ...
    public static void init() {
        ...
        CHLORINE =				new FluidType("CHLORINE",			0xBAB572, 3, 0, 0, EnumSymbol.OXIDIZER).addContainers(new CD_Gastank(0xBAB572, 0x887B34)).addTraits(GASEOUS, new FT_Corrosive(25)).addTraits(new FT_Toxin().addEntry(new ToxinDirectDamage(NtmDamageTypes.CLOUD, 2F, 20, HazardClass.GAS_LUNG, false)));
        ...
        PHOSGENE =				new FluidType("PHOSGENE",			0xCFC4A4, 4, 0, 1, EnumSymbol.NONE).addContainers(new CD_Gastank(0xCFC4A4, 0x361414)).addTraits(GASEOUS, new FT_Polluting().release(PollutionType.POISON, POISON_EXTREME)).addTraits(new FT_Toxin().addEntry(new ToxinDirectDamage(NtmDamageTypes.CLOUD, 4F, 20, HazardClass.GAS_LUNG, false)));
        MUSTARDGAS =			new FluidType("MUSTARDGAS",			0xBAB572, 4, 1, 1, EnumSymbol.NONE).addContainers(new CD_Gastank(0xBAB572, 0x361414)).addTraits(GASEOUS, new FT_Polluting().release(PollutionType.POISON, POISON_EXTREME)).addTraits(new FT_Toxin().addEntry(new ToxinDirectDamage(NtmDamageTypes.CLOUD, 4F, 10, HazardClass.GAS_BLISTERING, false)).addEntry(new ToxinEffects(HazardClass.GAS_BLISTERING, true).add(new MobEffectInstance(MobEffects.WITHER, 100, 1), new MobEffectInstance(MobEffects.CONFUSION, 100, 0))));
        ...
        REDMUD =				new FluidType("REDMUD",				0xD85638, 3, 0, 4, EnumSymbol.NONE).addTraits(LIQUID, VISCOUS, LEADCON, new FT_Corrosive(60), new FT_Flammable(1_000), new FT_Polluting().release(PollutionType.POISON, POISON_EXTREME)).addTraits(new FT_Toxin().addEntry(new ToxinEffects(HazardClass.GAS_BLISTERING, false).add(new MobEffectInstance(MobEffects.WITHER, 30 * 20, 2))));
        ...
        try {
            ...
            for (FluidType type : metaOrder) {
                ...
                for (Entry<Class<? extends FluidTrait>, FluidTrait> entry : type.traits.entrySet()) {
                    ...
                    String traitName = FluidTrait.traitNameMap.inverse().get(entry.getKey());

                    /* Ein Trait, den ein Fluid traegt, der aber nicht in FluidTrait registriert
                     * ist. Frueher gab es dafuer nur "NullPointerException: name == null" aus dem
                     * JsonWriter -- unbrauchbar, um die Ursache zu finden, und es riss den ganzen
                     * Mod beim Erststart mit. Jetzt fehlt eine Zeile in der Vorlage, und im
                     * Protokoll steht, welche Klasse nachzutragen ist. */
                    if(traitName == null) {
                        NuclearTechMod.LOGGER.error("Fluid {} traegt den Trait {}, der in FluidTrait nicht registriert ist -- er fehlt in der Vorlage. Bitte dort registerTrait(...) ergaenzen.",
                                type.getInternalName(), entry.getKey().getSimpleName());
                        continue;
                    }

                    writer.name(traitName).beginObject();
    ...
    private static void registerCalculatedFuel(FluidType type, double base, double combustMult, FuelGrade grade) {
        ...
        if(combustible > 0 && grade != null) type.addTraits(new FT_Combustible(grade, combustible));
    }
