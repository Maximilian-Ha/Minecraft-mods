...
/**
 * Portiert aus 1.7.10: com.hbm.inventory.gui.GUIMachinePrecAss.
 *
 * Bis auf die Textur, den Rezeptsatz und die zweite Faecherreihe dieselbe Oberflaeche wie die
 * Montagemaschine -- im Original ist sie eine Abschrift davon, und das bleibt so.
 */
public class MachinePrecAssScreen extends InfoScreen<MachinePrecAssMenu> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if(this.leftPos + 7 <= mouseX && this.leftPos + 7 + 18 > mouseX && this.topPos + 125 < mouseY && this.topPos + 125 + 18 >= mouseY) {
            if(this.be.assemblerModule.recipe != null && PrecAssRecipes.INSTANCE.recipeNameMap.containsKey(this.be.assemblerModule.recipe)) {
                GenericRecipe recipe = PrecAssRecipes.INSTANCE.recipeNameMap.get(this.be.assemblerModule.recipe);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if(this.isHovered(mouseX, mouseY, 7, 125, 18, 18)) RecipeSelectorScreen.openSelector(PrecAssRecipes.INSTANCE, be, be.assemblerModule.recipe, 0, BlueprintsItem.grabPool(be.slots.get(1)), this);
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        /* Linke Leuchte: Rezept gewaehlt. Gruen, sobald sie laeuft. */
        if(be.didProcess) {
