...
/**
 * Portiert aus 1.7.10: com.hbm.inventory.gui.GUIScreenRBMKLever.
 *
 * Je Hebel vier Eingabefelder: Kanal, Beschriftung, Befehl beim Einschalten, Befehl beim
 * Ausschalten. Dazu die beiden Schalter fuer sichtbar und staendig senden.
 *
 * Alle Koordinaten unveraendert.
 */
public class RBMKLeverScreen extends Screen {

    private static final ResourceLocation TEXTURE = NuclearTechMod.withDefaultNamespace("textures/gui/machine/gui_rbmk_lever.png");

    private static final int LEVERS = RBMKLeverBlockEntity.LEVERS;

    private final RBMKLeverBlockEntity lever;
    ...
    public RBMKLeverScreen(RBMKLeverBlockEntity lever) {
    ...
    protected void init() {
        ...
        for(int i = 0; i < LEVERS; i++) {
            ...
            LeverUnit unit = this.lever.levers[i];

            this.rtty[i] = this.field(this.leftPos + 27 + oX, this.topPos + 55 + oY + i * 54, 72 - oX * 2, RadioTorchScreen.MAX_CHAN_LENGTH, unit.rtty);
            this.label[i] = this.field(this.leftPos + 175 + oX, this.topPos + 55 + oY + i * 54, 72 - oX * 2, 15, unit.label);
            this.cmdOn[i] = this.field(this.leftPos + 45 + oX, this.topPos + 73 + oY + i * 54, 81 - oX * 2, 32, unit.commandOn);
            this.cmdOff[i] = this.field(this.leftPos + 166 + oX, this.topPos + 73 + oY + i * 54, 81 - oX * 2, 32, unit.commandOff);
    ...
    public void renderBackground(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        for(int i = 0; i < LEVERS; i++) {
            ...
            this.cmdOn[i].render(guiGraphics, mouseX, mouseY, partialTick);
            this.cmdOff[i].render(guiGraphics, mouseX, mouseY, partialTick);
        }
    }
    ...
    private void send() {
        ...
        for(int i = 0; i < LEVERS; i++) {
            ...
            tag.putString("cmdOn" + i, this.cmdOn[i].getValue());
            tag.putString("cmdOff" + i, this.cmdOff[i].getValue());
        }

        PacketDistributor.sendToServer(new CompoundTagControl(tag, this.lever.getBlockPos()));
    }
}
