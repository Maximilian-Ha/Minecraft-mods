...
public class UniNodespace {
    ...
    public static boolean checkConnection(GenNode connectsTo, DirPos connectFrom, boolean skipSideCheck) {
        for (DirPos revCon : connectsTo.connections) {
            // getStepX/Y/Z und getOppositeDir behandeln richtungslose Verbindungen (dir == null,
            // frueher ForgeDirection.UNKNOWN): Versatz 0 und null als eigene Gegenrichtung. Ohne das
            // wuerde hier jede Fernverbindung von Masten und Umspannwerk mit einer NPE abbrechen.
            if (revCon.getX() - revCon.getStepX() == connectFrom.getX() && revCon.getY() - revCon.getStepY() == connectFrom.getY() && revCon.getZ() - revCon.getStepZ() == connectFrom.getZ() && (revCon.getDir() == connectFrom.getOppositeDir() || skipSideCheck)) {
                return true;
            }
        }
        return false;
