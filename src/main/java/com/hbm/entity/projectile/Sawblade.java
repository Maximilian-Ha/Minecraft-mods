...
/**
 * Portiert aus 1.7.10: com.hbm.entity.projectile.EntitySawblade.
 *
 * Das Saegeblatt, das ein ueberdrehtes Saegewerk auswirft. Solange der
 * Orientierungswert unter 6 liegt, rollt es und zerlegt alles auf seinem Weg;
 * ab 6 liegt es flach und kann aufgesammelt werden. Die Zahl steht fuer die
 * Seitenkennziffer aus 1.7.10 (NORTH 2, SOUTH 3, WEST 4, EAST 5), damit der
 * Renderer unveraendert bleiben kann.
 */
public class Sawblade extends ProjectileNT {

    private static final EntityDataAccessor<Integer> ORIENTATION = SynchedEntityData.defineId(Sawblade.class, EntityDataSerializers.INT);

    /** Liegt das Blatt fest? Ersatz fuer EntityThrowableNT.inGround. */
    private boolean inGround = false;

    public Sawblade(EntityType<? extends Sawblade> type, Level level) {
        super(type, level);
    }

    public Sawblade(Level level, double x, double y, double z) {
        super(NtmEntityTypes.SAWBLADE.get(), level);
        this.setPos(x, y, z);
    }
    ...
    public Sawblade setOrientation(int rot) {
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        ...
        if(!this.level.isClientSide) {
            ...
            if(player.getInventory().add(new ItemStack(NtmItems.SAWBLADE.get()))) {
                this.discard();
            }
        }
    ...
    protected void onHitBlock(BlockHitResult bhr) {
        ...
        if(orientation < 6) {
            ...
            if(motion.length() < 0.75) {
                ...
                // Zerstoert wird nur der eine getroffene Block darunter, wenn seine
                // Sprengfestigkeit unter 50 liegt. Mit TNT waere daraus ein Krater geworden.
