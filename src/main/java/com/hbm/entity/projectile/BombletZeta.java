    ...
    public void tick() {
        ...
        if (!this.level().getBlockState(this.blockPosition()).isAir()) {
            if (!this.level().isClientSide) {
                ...
                // Typ 2 fehlte im Port ganz -- im Original wirft er eine Chlorwolke ab
                // (EntityBombletZeta.java:72-79).
                if (this.type == 2) {
                    this.level().playSound(null, this.getX() + 0.5F, this.getY() + 0.5F, this.getZ() + 0.5F,
                            SoundEvents.FIRE_EXTINGUISH, SoundSource.BLOCKS,
                            5.0F, 2.6F + (this.random.nextFloat() - this.random.nextFloat()) * 0.8F);

                    Mist mist = new Mist(this.level());
                    mist.setFluidType(Fluids.CHLORINE);
                    mist.setPos(this.getX() - this.getDeltaMovement().x, this.getY() - this.getDeltaMovement().y, this.getZ() - this.getDeltaMovement().z);
                    mist.setArea(15F, 7.5F);
