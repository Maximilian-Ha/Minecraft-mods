...
public class MovingItem extends MovingConveyorObject implements IConveyorItem {
    ...
    /** Rechtsklick nimmt den Gegenstand vom Band ins Inventar. */
    @Override
    public InteractionResult interact(Player player, InteractionHand hand) {

        if(this.level.isClientSide || this.isRemoved()) return InteractionResult.PASS;

        if(player.getInventory().add(this.getItemStack().copy())) {
            this.discard();
    ...
    /**
     * Ein Schlag wirft den Gegenstand vom Band -- ob von einem Spieler oder von einer Explosion,
     * macht keinen Unterschied. Im Original stehen dafuer zwei Methoden nebeneinander,
     * hitByEntity und attackEntityFrom; auf 1.21 laeuft beides durch hurt.
     */
    @Override
    ...
    /**
     * Am Ende der Strecke steht eine Maschine, die Gegenstaende annimmt. Nimmt sie diesen an,
     * verschwindet die fahrende Entitaet -- der Gegenstand ist dann drin.
     */
    @Override
    public void enterBlock(IEnterableBlock enterable, BlockPos pos, Direction dir) {
        ...
        if(enterable.canItemEnter(this.level, pos, dir, this)) {
            enterable.onItemEnter(this.level, pos, dir, this);
            this.discard();
        }
    }
