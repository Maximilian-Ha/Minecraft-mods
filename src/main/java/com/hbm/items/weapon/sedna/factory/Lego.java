...
public class Lego {
    ...
    /**
     * Der Feuerwahlschalter: im Leerlauf zwischen Einzel- und Dauerfeuer umlegen. Welcher Modus
     * was bedeutet, entscheidet die Waffe selbst -- hier wird nur zwischen 0 und 1 gewechselt und
     * das passende Klacken gespielt.
     *
     * ABWEICHUNG: das Original liest den alten Modus immer aus Konfiguration 0, schreibt ihn aber
     * in die aktuelle zurueck. Bei einer beidhaendigen Waffe legt der rechte Schalter damit um,
     * was am linken steht. Hier wird durchgaengig die aktuelle Konfiguration gelesen.
     */
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_STANDARD_CLICK_SECONDARY = (stack, ctx) -> {
        ...
        if(state == GunState.IDLE) {
            int mode = GunBaseNTItem.getMode(stack, index);
            GunBaseNTItem.setMode(stack, index, 1 - mode);
            SoundUtils.playAtVec3(entity.level, entity.position(),
                    (mode == 0 ? NtmSoundEvents.GUN_SWITCHMODE_1 : NtmSoundEvents.GUN_SWITCHMODE_2).get(),
...
//            }
    ...
    /**
     * Die kleine Sprengwirkung. Sie reisst ein halbes Loch statt eines ganzen und wirft das Ziel
     * nur leicht zurueck -- fuer die Sprenggeschosse der Handwaffen, die keine Panzerung knacken
     * sollen, sondern Menschenansammlungen.
     *
     ...
     */
    public static void tinyExplode(BulletBaseMK4 bullet, HitResult hr, float range) { tinyExplode(bullet, hr, range, 1F); }
    public static void tinyExplode(BulletBaseMK4 bullet, HitResult hr, float range, float damageMod) {
        Vec3 position = hr.getLocation();
        ExplosionVNT vnt = new ExplosionVNT(bullet.level, position.x, position.y, position.z, range, bullet.getOwner())
