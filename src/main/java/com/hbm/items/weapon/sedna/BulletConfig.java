...
public class BulletConfig implements Cloneable {
    ...
    /**
     * Der Aufschlag eines Strahls. Er ist dem gewoehnlichen Aufschlag aehnlich, aber einfacher:
     * ein Strahl durchschlaegt nicht, prallt nicht ab und kennt keinen Kopftreffer -- er trifft
     * oder trifft nicht.
     */
    public static BiConsumer<BulletBeamBase, HitResult> LAMBDA_STANDARD_BEAM_HIT = (beam, hr) -> {

        if(!(hr instanceof EntityHitResult ehr)) return;

        Entity entity = ehr.getEntity();
        if(entity instanceof LivingEntity dying && dying.isDeadOrDying()) return;

        DamageSource source = getDamage(beam.level(), entity, beam.getThrower(), beam.config.dmgClass);

        if(!(entity instanceof LivingEntity living)) {
            EntityDamageUtil.hurtIgnoreIFrame(entity, source, beam.damage);
            return;
        }

        EntityDamageUtil.hurtNT(living, source, beam.damage, true, true,
                beam.config.knockbackMult, beam.config.armorThresholdNegation, beam.config.armorPiercingPercent);

        if(!living.isAlive()) ConfettiUtil.createConfetti(living, beam.config.dmgClass);
    };
