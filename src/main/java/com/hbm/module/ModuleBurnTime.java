...
public class ModuleBurnTime {
    ...
    /**
     * Konfiguration, siehe {@link com.hbm.config.MachineDynConfig}. Anders als bei den Maschinen
     * sind das hier keine statischen Werte, sondern die des jeweiligen Moduls -- Brennkasten und
     * Backofen haben eigene Instanzen und damit eigene Unterobjekte in der Datei.
     *
     * Der Schluessel "D:heatBalefie" ist im Original verschrieben. Er bleibt so, sonst laesst
     * sich eine vorhandene hbmMachines.json nicht weiterverwenden.
     */
    public void readIfPresent(JsonObject obj) {
        modTime[MOD_LOG] = IConfigurableMachine.grab(obj, "D:timeLog", modTime[MOD_LOG]);
        modTime[MOD_WOOD] = IConfigurableMachine.grab(obj, "D:timeWood", modTime[MOD_WOOD]);
        modTime[MOD_COAL] = IConfigurableMachine.grab(obj, "D:timeCoal", modTime[MOD_COAL]);
        modTime[MOD_LIGNITE] = IConfigurableMachine.grab(obj, "D:timeLignite", modTime[MOD_LIGNITE]);
        modTime[MOD_COKE] = IConfigurableMachine.grab(obj, "D:timeCoke", modTime[MOD_COKE]);
        modTime[MOD_SOLID] = IConfigurableMachine.grab(obj, "D:timeSolid", modTime[MOD_SOLID]);
        modTime[MOD_ROCKET] = IConfigurableMachine.grab(obj, "D:timeRocket", modTime[MOD_ROCKET]);
        modTime[MOD_BALEFIRE] = IConfigurableMachine.grab(obj, "D:timeBalefire", modTime[MOD_BALEFIRE]);

        modHeat[MOD_LOG] = IConfigurableMachine.grab(obj, "D:heatLog", modHeat[MOD_LOG]);
        modHeat[MOD_WOOD] = IConfigurableMachine.grab(obj, "D:heatWood", modHeat[MOD_WOOD]);
        modHeat[MOD_COAL] = IConfigurableMachine.grab(obj, "D:heatCoal", modHeat[MOD_COAL]);
        modHeat[MOD_LIGNITE] = IConfigurableMachine.grab(obj, "D:heatLignite", modHeat[MOD_LIGNITE]);
        modHeat[MOD_COKE] = IConfigurableMachine.grab(obj, "D:heatCoke", modHeat[MOD_COKE]);
        modHeat[MOD_SOLID] = IConfigurableMachine.grab(obj, "D:heatSolid", modHeat[MOD_SOLID]);
        modHeat[MOD_ROCKET] = IConfigurableMachine.grab(obj, "D:heatRocket", modHeat[MOD_ROCKET]);
        modHeat[MOD_BALEFIRE] = IConfigurableMachine.grab(obj, "D:heatBalefie", modHeat[MOD_BALEFIRE]);
    }

    public void writeConfig(JsonWriter writer) throws IOException {
        writer.name("D:timeLog").value(modTime[MOD_LOG]);
        writer.name("D:timeWood").value(modTime[MOD_WOOD]);
        writer.name("D:timeCoal").value(modTime[MOD_COAL]);
        writer.name("D:timeLignite").value(modTime[MOD_LIGNITE]);
        writer.name("D:timeCoke").value(modTime[MOD_COKE]);
        writer.name("D:timeSolid").value(modTime[MOD_SOLID]);
        writer.name("D:timeRocket").value(modTime[MOD_ROCKET]);
        writer.name("D:timeBalefire").value(modTime[MOD_BALEFIRE]);
        writer.name("D:heatLog").value(modHeat[MOD_LOG]);
        writer.name("D:heatWood").value(modHeat[MOD_WOOD]);
        writer.name("D:heatCoal").value(modHeat[MOD_COAL]);
        writer.name("D:heatLignite").value(modHeat[MOD_LIGNITE]);
        writer.name("D:heatCoke").value(modHeat[MOD_COKE]);
        writer.name("D:heatSolid").value(modHeat[MOD_SOLID]);
        writer.name("D:heatRocket").value(modHeat[MOD_ROCKET]);
        writer.name("D:heatBalefie").value(modHeat[MOD_BALEFIRE]);
    }
